...
public class DruidSelectParser extends DefaultDruidParser {
    ...
    @Override
	public void statementParse(Configurations configurations,
			RouteResultset rrs, SQLStatement stmt) {
		...
		if(sqlSelectQuery instanceof MySqlSelectQueryBlock) {
			...
			MySqlSelectQueryBlock mysqlSelectQuery = (MySqlSelectQueryBlock)selectStmt.getSelect().getQuery();
				 parseOrderAggGroupMysql(configurations, stmt,rrs, mysqlSelectQuery);
...
//			System.out.println();
	...
	protected void parseOrderAggGroupMysql(Configurations configurations, 
			SQLStatement stmt,RouteResultset rrs, MySqlSelectQueryBlock mysqlSelectQuery){
		  	if(!isNeedParseOrderAgg){
	            return;
	        }
			Map<String, String> aliaColumns = parseAggGroupCommon(configurations, stmt, rrs, mysqlSelectQuery);
			//setOrderByCols
	...
	protected Map<String, String> parseAggGroupCommon(Configurations configurations, SQLStatement stmt, RouteResultset rrs, SQLSelectQueryBlock mysqlSelectQuery){
        ...
        for (int i = 0; i < size; i++){
			...
			if (item.getExpr() instanceof SQLAggregateExpr){
                ...
                if (MergeCol.MERGE_AVG == mergeType&&isRoutMultiNode(configurations,rrs)){//???????????? 
	...
	private boolean isRoutMultiNode(Configurations configurations,
			RouteResultset rrs) {

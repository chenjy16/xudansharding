 ...
 */
public class DruidSchemaStatVisitor extends MySqlSchemaStatVisitor {
 		...
 		for(WhereUnit whereUnit : whereUnitList) {
 			if(whereUnit.getSplitedExprList().size() > 0) {
 				...
 				for(SQLExpr sqlExpr : whereUnit.getSplitedExprList()) {//
 					reset();
 					if(isExprHasOr(sqlExpr)) {//3、这里解析
 						removeSplitedList.add(sqlExpr);
 						WhereUnit subWhereUnit = this.whereUnits.get(0);//4、取得对象
 						splitUntilNoOr(subWhereUnit);//5、拆分
	...
	* @throws
	...
	private void getConditionsFromWhereUnit(WhereUnit whereUnit) {
		...
		for(SQLExpr sqlExpr : whereUnit.getSplitedExprList()) {
			...
			conditions.addAll(getConditions());//存储内层的
			conditions.addAll(outSideCondition);//存储外层的
	...
	private void splitUntilNoOr(WhereUnit whereUnit) {
		...
		if(whereUnit.isFinishedParse()) {
			...
			if(whereUnit.getSubWhereUnit().size() > 0) {//有嵌套的
			...
			if(expr.getOperator() == SQLBinaryOperator.BooleanOr) {//有or关键字
				
				addExprIfNotFalse(whereUnit, expr.getRight());//最右侧的嵌套无法解析
				if(expr.getLeft() instanceof SQLBinaryOpExpr) {//二元操作符能否进一步拆分
